...
import { ethers } from "ethers"; // Import ethers from the correct package
...
export default function Home() {
  ...
  async function loadNFTs() {
    const provider = new ethers.providers.JsonRpcProvider(); // Correctly initialize JsonRpcProvider
  ...
  if (loadingState === "loaded" && nfts.length === 0) {
    // Fixed the condition for checking if nfts is empty
    return (
    ...
    <div className="flex justify-center">
      <div className="px-4" style={{ maxWidth: "1600px" }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 pt-4">
          {" "}
          {/* Fixed the typo in lg_grid-cols-4 */}
          {nfts.map((nft, i) => (
